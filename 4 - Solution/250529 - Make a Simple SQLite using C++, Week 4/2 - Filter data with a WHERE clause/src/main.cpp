...
int main(int argc, char* argv[])
{
    ...
    {
        int select_idx = command.find("SELECT");
        int from_idx = command.find("FROM");
        int where_idx = command.find("WHERE");
        ...
        else
        {
            ...
            if (where_idx != -1)
            {
                ...
                where_column = where_clause[0];
                where_arg =
                    where_clause[2].substr(1, where_clause[2].size() - 2);
            }

            create_statement.pop_back();

            std::vector<std::string> columns = split(
                create_statement.substr(15 + target_table_name.size()), ',');
